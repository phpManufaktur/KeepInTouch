...
/**
  ...
  $Id: tool.php 49 2010-08-06 05:03:59Z ralf $
   ...
   */
	public function dlgList() {
		...
		case self::action_list_city: 
			...
			$SQL = sprintf(	"SELECT %s.%s,%s,%s,%s,%s,%s,%s,%s,%s FROM %s, %s WHERE %s!='%s' AND %s!='' AND %s=%s ORDER BY %s ASC",
		...
		case self::action_list_company:
			...
			$SQL = sprintf(	"SELECT %s.%s,%s,%s,%s,%s,%s,%s,%s,%s FROM %s WHERE %s!='%s' AND %s!='' ORDER BY %s ASC", 
											$dbContact->getTableName(),
											dbKITcontact::field_id,
											dbKITcontact::field_person_last_name,
											dbKITcontact::field_person_first_name,
		...
		foreach ($list_array as $contact) {
			...
			// Edit
			$edit = sprintf('<a href="%s"><img src="%s" width="%s" height="%s" alt="%s" title="%s" /></a>',
											$this->page_link.'&amp;'.self::request_action.'='.self::action_contact.'&amp;'.dbKITcontact::field_id.'='.$contact[dbKITcontact::field_id],
											WB_URL.'/modules/'.basename(dirname(__FILE__)).'/img/edit.gif',
											16, 16,
											kit_label_contact_edit,
											kit_label_contact_edit);
			// Street and City
  	...
  	if (empty($item[dbKITcontact::field_address])) $item[dbKITcontact::field_address] = -1;
		...
		foreach ($address_array as $addr) {
			...
			if (!$skip) {
				...
				$street = sprintf('<input type="text" name="%s" value="%s" />',
													dbKITcontactAddress::field_street.'_'.$addr_values[dbKITcontactAddress::field_id], 
													$addr_values[dbKITcontactAddress::field_street]);
				$zip = sprintf('<input type="text" name="%s" value="%s" />',
												dbKITcontactAddress::field_zip.'_'.$addr_values[dbKITcontactAddress::field_id],
												$addr_values[dbKITcontactAddress::field_zip]);
				$city = sprintf('<input type="text" name="%s" value="%s" />',
												dbKITcontactAddress::field_city.'_'.$addr_values[dbKITcontactAddress::field_id],
				...
				$addr_standard = sprintf(	'<input type="radio" name="%s" value="%s"%s />%s', 
																	dbKITcontact::field_address_standard, 
																	$addr_values[dbKITcontactAddress::field_id], 
																	$checked,
																	kit_label_standard);
		...
		$birthday = sprintf('<input type="text" id="%s" name="%s" value="%s" />',
												'datepicker', dbKITcontact::field_birthday, $date);
		// contact since
		$timestamp = -1;
		...
		$contact_since = sprintf(	'<input type="text" id="%s" name="%s" value="%s" />',
															'datepicker_2', dbKITcontact::field_contact_since, $date);
		// contact note
		...
		foreach ($email_array as $email_item) {
			...
			$email_address = sprintf(	'<input type="text" name="%s" value="%s" /><input type="radio" name="%s" value="%s"%s />', 
																dbKITcontact::field_email.'_'.$i, 
																$email_address,
																dbKITcontact::field_email_standard,
																$i,
																$checked);
		...
		foreach ($phone_array as $phone_item) {
			...
			$phone_number = sprintf('<input type="text" id="%s" name="%s" value="%s" /><input type="radio" name="%s" value="%s"%s />',
															dbKITcontact::field_phone.'_'.$i,
															dbKITcontact::field_phone.'_'.$i,
															$phone_number,
															dbKITcontact::field_phone_standard,
															$i,
		...
		foreach ($internet_array as $internet_item) {
			...
			$internet_address = sprintf('<input type="text" name="%s" value="%s" />',
															dbKITcontact::field_internet.'_'.$i,
															$internet_address);
			$data = array(
		...
		if (!empty($map_address)) { 
			...
			switch ($map_pos):
			case 'l': $pos = 'float:left;'; break;
			case 'r': $pos = 'float:right;'; break;
			default: $pos = '';
			endswitch;
		...
		$protocol_date = sprintf(	'<input type="text" id="%s" name="%s" value="%s" />',
															'datepicker_3', dbKITprotocol::field_date, $date);
	...
	private function checkContact() {
		...
		if (isset($_REQUEST['address_array'])) {
			...
			foreach ($address_array as $addr_id) {
				...
				$zip_ok = false;
				$city_ok = false;
				if ((isset($_REQUEST[dbKITcontactAddress::field_street.'_'.$addr_id])) &&
						(!empty($_REQUEST[dbKITcontactAddress::field_street.'_'.$addr_id]))) $street_ok = true;
				if ((isset($_REQUEST[dbKITcontactAddress::field_zip.'_'.$addr_id])) &&
						(!empty($_REQUEST[dbKITcontactAddress::field_zip.'_'.$addr_id]))) $zip_ok = true;
				if ((isset($_REQUEST[dbKITcontactAddress::field_city.'_'.$addr_id])) &&
						(!empty($_REQUEST[dbKITcontactAddress::field_city.'_'.$addr_id]))) $city_ok = true;
...
*/		
		...
		if (empty($_REQUEST[dbKITcontact::field_contact_identifier])) { 
			...
			if (!empty($this->contact_array[dbKITcontact::field_company_name]) && 
					!empty($this->contact_array[dbKITcontact::field_person_last_name])) { 
				// Firmenname + Nachname
				$this->contact_array[dbKITcontact::field_contact_identifier] = 
					sprintf('%s, %s %s', 
									$this->contact_array[dbKITcontact::field_company_name],
									$dbKITcontact->person_title_array[$this->contact_array[dbKITcontact::field_person_title]],
									$this->contact_array[dbKITcontact::field_person_last_name]);		
			}
			elseif (!empty($this->contact_array[dbKITcontact::field_company_name])) {
				...
				if (!empty($this->contact_array[dbKITcontact::field_company_department])) {
					$this->contact_array[dbKITcontact::field_contact_identifier] =
						sprintf('%s, %s', 
										$this->contact_array[dbKITcontact::field_company_name],
										$this->contact_array[dbKITcontact::field_company_department]);
				}
				else {
			...
			elseif (!empty($this->contact_array[dbKITcontact::field_person_last_name])) {
				if (!empty($this->contact_array[dbKITcontact::field_person_first_name])) {
					$this->contact_array[dbKITcontact::field_contact_identifier] =
						sprintf('%s, %s',
							$this->contact_array[dbKITcontact::field_person_last_name],
							$this->contact_array[dbKITcontact::field_person_first_name]);
				}
				else {
	...
	private function saveContact() {
		 ...
		 * Name+Telefon
		 */ 
		if ((empty($_REQUEST[dbKITcontact::field_email.'_0'])) && 
				(empty($_REQUEST[dbKITcontact::field_company_name]) && empty($_REQUEST[dbKITcontactAddress::field_city])) &&
				(empty($_REQUEST[dbKITcontact::field_company_name]) && empty($_REQUEST[dbKITcontact::field_phone])) &&
				(empty($_REQUEST[dbKITcontact::field_person_last_name]) && empty($_REQUEST[dbKITcontactAddress::field_city])) &&
				(empty($_REQUEST[dbKITcontact::field_person_last_name]) && empty($_REQUEST[dbKITcontact::field_phone]))) {
		...
		if ($this->contact_array[dbKITcontact::field_id] == -1) {
			...
			if ($this->contact_array[dbKITcontact::field_status] == dbKITcontact::status_deleted) {
				...
				$sql = sprintf(	"UPDATE %s SET %s='%s', %s='%s', %s='%s' WHERE %s='%s' AND %s!='%s'",
	...
	private function dlgConfigGeneral() {
		...
		$SQL = sprintf(	"SELECT * FROM %s WHERE NOT %s='%s' ORDER BY %s",
										$dbCfg->getTableName(),
										dbKITcfg::field_status,
										dbKITcfg::status_deleted,
										dbKITcfg::field_label);
		$config = array();
...
//				'type'				=> $typ,
		...
		$items .= $parser->get($this->template_path.'backend.config.general.csv.htt', array('name' => self::request_csv_export, 'label' => kit_label_csv_export));
		// Mitteilungen anzeigen
		if ($this->isMessage()) {
			$intro = sprintf('<div class="message">%s</div>', $this->getMessage());
		}
	...
	public function dlgConfigArray() {
		...
		$SQL = sprintf(	"SELECT * FROM %s WHERE %s!='%s' ORDER BY %s, %s",
										$dbContactArrayCfg->getTableName(),
										dbKITcontactArrayCfg::field_status,
										dbKITcontactArrayCfg::status_deleted,
		...
		foreach ($config_array as $item) { 
			...
			// value
			$ivalue = sprintf('<input type="text" name="%s" value="%s" />', 
												dbKITcontactArrayCfg::field_value.'_'.$item[dbKITcontactArrayCfg::field_id], 
												$item[dbKITcontactArrayCfg::field_value]);
			// identifier
			...
			$status = sprintf('<select name="%s">%s</select>', 
												dbKITcontactArrayCfg::field_status.'_'.$item[dbKITcontactArrayCfg::field_id], 
												$option);
			// last change
			$last_change = sprintf(	'%s, %s',
															$item[dbKITcontactArrayCfg::field_update_by],
															date(DATE_FORMAT.' - '.TIME_FORMAT, strtotime($item[dbKITcontactArrayCfg::field_update_when])));
			$data = array(
		...
		for ($i=0; $i < 3; $i++) {
			...
			$type = sprintf('<select name="%s">%s</select>', 
												dbKITcontactArrayCfg::field_type.'_add_'.$i, 
												$option);
	...
	public function dlgConfigImport() {
		...
		if ($dbMassmail->sqlTableExists()) {
			...
			$items .= $parser->get(	$this->template_path.'backend.config.import.massmail.tr.htt', 
																array('label' => kit_label_massmail, 'value' => kit_msg_massmail_not_installed));
		}		
  	// Mitteilungen anzeigen
		if ($this->isMessage()) {
			$intro = sprintf('<div class="message">%s</div>', $this->getMessage());
	...
	public function execImportMassmail() {
		...
		if (count($massmail_data) < 1) {
			...
			foreach ($massmail_data as $massmail) {
				$SQL = sprintf(	"SELECT %s FROM %s WHERE %s LIKE '%%%s%%'", 
												dbKITcontact::field_id,
												$dbContact->getTableName(),
												dbKITcontact::field_email,
												$massmail[dbMassmailAddresses::field_mail_to]);
				$kitCheck = array();
	...
	public function dlgConfig() {
  	...
  	foreach ($this->tab_config_array as $key => $value) {
  		...
  		$cfg_tab .= sprintf(	'<li%s><a href="%s">%s</a></li>', 
	  												$selected,
	  												sprintf('%s&%s=%s&%s=%s', $this->page_link, self::request_action, self::action_cfg, self::request_cfg_tab, $key),
	  												$value
	  												);
  	}
	...
	public function saveConfigArray() {
		...
		if (isset($_REQUEST['item_count'])) {
			...
			foreach ($item_array as $id) {
				...
				if (($req_value != $data[dbKITcontactArrayCfg::field_value]) ||
						($_REQUEST[dbKITcontactArrayCfg::field_status.'_'.$id] != $data[dbKITcontactArrayCfg::field_status])) {
					// Datensatz geaendert
					$data = array();
		...
		for ($i=0; $i < 3; $i++) {
			if (isset($_REQUEST[dbKITcontactArrayCfg::field_value.'_add_'.$i]) && 
					!empty($_REQUEST[dbKITcontactArrayCfg::field_value.'_add_'.$i]) &&
					!empty($_REQUEST[dbKITcontactArrayCfg::field_identifier.'_add_'.$i])) {
				...
				if (count($check) > 0) {
					...
					$message .= sprintf(kit_msg_cfg_array_identifier_in_use, 
															$_REQUEST[dbKITcontactArrayCfg::field_identifier.'_add_'.$i], 
															$check[0][dbKITcontactArrayCfg::field_id]);
				}
				else {
	...
	public function saveConfigGeneral() {
		...
		if ((isset($_REQUEST[dbKITcfg::field_name])) && (!empty($_REQUEST[dbKITcfg::field_name]))) {
			...
			if (sizeof($result) > 0) {
				...
				if (((isset($_REQUEST[dbKITcfg::field_type])) && ($_REQUEST[dbKITcfg::field_type] != dbKITcfg::type_undefined)) &&
						((isset($_REQUEST[dbKITcfg::field_value])) && (!empty($_REQUEST[dbKITcfg::field_value]))) &&
						((isset($_REQUEST[dbKITcfg::field_label])) && (!empty($_REQUEST[dbKITcfg::field_label]))) &&
						((isset($_REQUEST[dbKITcfg::field_description])) && (!empty($_REQUEST[dbKITcfg::field_description])))) {
	...
	public function dlgConfigProvider() {
		...
		$SQL = sprintf(	"SELECT %s, %s FROM %s WHERE %s != '%s' ORDER BY %s ASC",
	...
	public function saveConfigProvider() {
		...
		if ((((bool) $new[dbKITprovider::field_smtp_auth]) && 
				 (empty($_REQUEST[dbKITprovider::field_name]) ||
				  empty($_REQUEST[dbKITprovider::field_identifier]) ||
					empty($_REQUEST[dbKITprovider::field_email]) ||
					empty($_REQUEST[dbKITprovider::field_smtp_host]) ||
					empty($_REQUEST[dbKITprovider::field_smtp_user]))) ||
				(!(bool) $new[dbKITprovider::field_smtp_auth]) &&
				 (empty($_REQUEST[dbKITprovider::field_name])||
				 	empty($_REQUEST[dbKITprovider::field_identifier]) ||
				  empty($_REQUEST[dbKITprovider::field_email]))) {
...
*/
	...
	public function dlgEMail() {
		...
		$SQL = sprintf(	"SELECT * FROM %s WHERE %s='%s' AND %s='%s' ORDER BY %s ASC",
										$dbContactArrayCfg->getTableName(),
										dbKITcontactArrayCfg::field_type,
										dbKITcontactArrayCfg::type_category,
										dbKITcontactArrayCfg::field_status,
		...
		foreach ($category_array as $category) {
			$SQL = sprintf(	"SELECT COUNT(%s) FROM %s WHERE %s='%s' AND ((%s LIKE '%s') OR (%s LIKE '%s,%%') OR (%s LIKE '%%,%s') OR (%s LIKE '%%%s,%%'))",
											...
											dbKITcontact::field_category,
											$category[dbKITcontactArrayCfg::field_identifier],
											dbKITcontact::field_category,
											$category[dbKITcontactArrayCfg::field_identifier],
											dbKITcontact::field_category,
											$category[dbKITcontactArrayCfg::field_identifier],
			...
			$cat .= sprintf('<input type="checkbox" name="%s[]" value="%s"%s /> %s (<b>%d</b> %s)<br />',
											dbKITcontact::field_category,
											$category[dbKITcontactArrayCfg::field_identifier],
											$checked,
											$category[dbKITcontactArrayCfg::field_value],
											$result[0]['COUNT('.dbKITcontact::field_id.')'],
											kit_text_records);											
		} // foreach
		$items .= $parser->get($row, array('label' => kit_label_audience, 'value' => $cat));
   	...
   	// get all newsletters
		...
		$items .= $parser->get($row, array(	'label' => kit_label_html_format, 'value' => $html));
	...
	public function sendEMail() {
		...
		if ((!isset($_REQUEST[dbKITcontact::field_category]) &&
				...
				empty($_REQUEST[dbKITmail::field_subject]) ||
				empty($_REQUEST[dbKITmail::field_html])) {
			// missing values...
