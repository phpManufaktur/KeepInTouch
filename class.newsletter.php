...
class kitNewsletterCommands {
  ...
  public $cmd_array = array(
    self::cmd_account_email							=> 'E-Mail Address',
    self::cmd_account_first_name				=> 'First name',
    self::cmd_account_id								=> 'ID of the Subscriber (Account)',
    self::cmd_account_last_name					=> 'Last name',
    self::cmd_account_register_key			=> 'Registered subscribers of the key account',
    self::cmd_account_title							=> 'Title of subscribers (Mr. or Ms.)',
    self::cmd_account_title_academic		=> 'Title of the subscriber (for example, Dr. or Prof.)',
    self::cmd_account_username					=> 'User name of the subscriber (user name)',
    self::cmd_contact_id								=> 'ID of the entry in the Contact Database (Contact)',
    self::cmd_content										=> 'The actual text of the newsletter will be inserted at this point',
    self::cmd_kit_info									=> 'Gives information about KeepInTouch (KIT) from',
    self::cmd_kit_release								=> 'Returns the release number of KeepInTouch (KIT) from',
    self::cmd_salutation_01							=> 'Greetings 1-10, the salutation variations are defined in the preferences (Salutation)',
    self::cmd_salutation_02							=> 'Greetings 1-10, the salutation variations are defined in the preferences (Salutation)',
    self::cmd_salutation_03							=> 'Greetings 1-10, the salutation variations are defined in the preferences (Salutation)',
    self::cmd_salutation_04							=> 'Greetings 1-10, the salutation variations are defined in the preferences (Salutation)',
    self::cmd_salutation_05							=> 'Greetings 1-10, the salutation variations are defined in the preferences (Salutation)',
    self::cmd_salutation_06							=> 'Greetings 1-10, the salutation variations are defined in the preferences (Salutation)',
    self::cmd_salutation_07							=> 'Greetings 1-10, the salutation variations are defined in the preferences (Salutation)',
    self::cmd_salutation_08							=> 'Greetings 1-10, the salutation variations are defined in the preferences (Salutation)',
    self::cmd_salutation_09							=> 'Greetings 1-10, the salutation variations are defined in the preferences (Salutation)',
    self::cmd_salutation_10							=> 'Greetings 1-10, the salutation variations are defined in the preferences (Salutation)'
  );
...
class kitNewsletterDialog {
  ...
  public function __construct() {
    ...
    $this->temp_path = WB_PATH.'/temp/kit/newsletter/';
  } // __construct()
  ...
  public function dlgTemplate() {
    ...
    $data = array(
      ...
      'btn_preview'							=> $this->lang->translate('Preview'),
  ...
  public function createTemplatePreview($preview_id, $is_html=true) {
    ...
    if (count($preview) < 1) {
      // Datensatz nicht gefunden
      $this->setError('[%s - %s] %s', __METHOD__, __LINE__, sprintf(kit_error_item_id, $preview_id));
      return false;
    }
    ...
    if (!file_exists($this->temp_path)) {
      if (!mkdir($this->temp_path, 0755, true)) {
        $this->setError(sprintf('[%s - %s] %s', __METHOD__, __LINE__, sprintf(kit_error_create_dir, substr($this->temp_path, strlen(WB_PATH)))));
        return false;
      }
    }
    if ($newsletterCommands->parseCommands($template, '', -1)) {
      if (!$is_html) {
        // NUR TEXT
        if (!file_put_contents($this->temp_path.'nl-preview.txt', strip_tags($template))) {
        ...
        // HTML
        if (!file_put_contents($this->temp_path.'nl-preview.html', $template)) {
   ...
   *
   * @return STR Dialog Preview
   */
  public function dlgCheckTemplatePreview($preview_id) {
    global $dbCfg;
    global $parser;

    if (!$this->createTemplatePreview($preview_id, true)) return false;
    if (!$this->createTemplatePreview($preview_id, false)) return false;
    ...
    $data = array(
      ...
      'preview_name'							=> dbKITnewsletterPreview::field_id,
      'preview_value'							=> $preview_id,
      'btn_save'									=> $this->lang->translate('Save'),
      'action_edit_value'					=> self::action_template,
      'btn_edit'									=> $this->lang->translate('Edit'),
      'btn_abort'									=> $this->lang->translate('Abort'),
      'abort_location'						=> $this->page_link
    );
    ...
    if (count($newsletter_archive) > 0) {
      ...
      foreach ($newsletter_archive as $item) {
        $option .= sprintf(	'<option value="%s">[%04d] %s - %s',
                            ...
                            $item[dbKITnewsletterArchive::field_id],
                            date(CFG_DATETIME_STR, strtotime($item[dbKITnewsletterArchive::field_update_when])),
  ...
  public function dlgCheckNewsletterPreview() {
    ...
    $data = array(
      ...
      'btn_save'									=> $this->lang->translate('Send'),
    ...
    if (count($process) < 1) {
      ...
      foreach ($process as $job) {
        ...
        $data = array(
          ...
          'created'			=> date(CFG_DATETIME_STR, strtotime($job[dbKITnewsletterProcess::field_update_when])),
